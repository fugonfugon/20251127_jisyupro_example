            ...
            if state == "SEARCH":
                ...
                if lost:
                    ...
                    # APPPROACH：中央を維持しつつ「前へ」近づく
                        ...
                        # ---- ★変更点②：bottleが近づいたら掴む ----
                        # rho_target に達したとき、一度だけグリッパを閉じる
                        if not hasattr(auto_track_loop, "gripper_closed"):
                            auto_track_loop.gripper_closed = False

                        if not auto_track_loop.gripper_closed and (rho >= cfg["rho_target"]):
                            print("[CTRL] Target close enough. Closing gripper to grasp bottle...")
                            target_positions["gripper"] = -20.0  # 閉じる方向（角度は調整）
                            robot.send_action({"gripper.pos": -20.0})
                            auto_track_loop.gripper_closed = True
                            time.sleep(1.0)
...
def main():
    ...
    try:
        ...
        # ---- ★変更点①：起動時に手を開く ----
        print("Opening gripper at start...")
        robot.send_action({"gripper.pos": 30.0})  # 開いた状態（角度は調整可）
        time.sleep(1.0)
